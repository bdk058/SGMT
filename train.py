...
class Train:
    ...
    def set_scheduler(self):
        if 'lr_scheduler' in self.config['train_params'] and self.config['train_params']['lr_scheduler'] != {}:
            ...
            warmup_steps = min(int(self.config['train_params']['lr_scheduler']['warmup'] * n_iter_per_epoch),
                   int(0.1 * num_steps))  # warmup tối đa 10% tổng số bước

            # 🩶 fix lỗi warmup >= total steps
            if warmup_steps >= num_steps:
                print(f"[Warning] warmup_steps ({warmup_steps}) >= total steps ({num_steps}), adjusting automatically.")
...
class CosineAnnealingWarmupRestarts(_LRScheduler):
    def __init__(
        ...
        assert warmup_steps < first_cycle_steps, \
            f"warmup_steps ({warmup_steps}) must be smaller than first_cycle_steps ({first_cycle_steps})"
